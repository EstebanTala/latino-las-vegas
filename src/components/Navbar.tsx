...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-[200] bg-[rgba(5,5,5,0.5)] backdrop-blur-lg border-b border-white/[0.08] transition-all">
      <div className="flex items-center justify-between px-6 md:px-12 h-[52px]">
        {/* Logo */}
        ...
        <div className="hidden md:flex items-center gap-8">
          <div className="flex items-center gap-7">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="font-condensed text-[13px] font-bold tracking-[1.5px] uppercase text-dark-text-2 hover:text-dark-text transition-colors"
              >
                {link.label}
              </Link>
            ))}
          ...
          <Link
            href="/anunciate"
            className="font-condensed text-xs font-bold tracking-[1px] uppercase px-4 py-1.5 rounded-sm bg-red text-primary-foreground shadow-[0_2px_8px_hsl(var(--red)/0.25)] hover:bg-red-light hover:-translate-y-px transition-all"
          >
            Agrega Tu Negocio
          </Link>
        </div>
